    ...
    def get_content(self):
        ...
        with open("compony7.txt", "r")as f:
            ...
            for line in f.readlines():
                ...
                try:
                    response = s.get(self.url.format(line.split("\n")[0]), headers=self.headers)
        ...
        # logger.info("共爬取公司/项目条数为%s" % self.company_times)
        # 获取投融资速递
        ...
        # logger.info("共爬取投融资速递条数为%s" % self.f_times)
        ...
        # logger.info("共爬取投资机构条数为%s" % self.m_times)
        # 开始爬投资人物
        logger.info("开始爬创投人物")
        ...
        while True:
            ...
            if len(response) > 100:
                ...
                for data in data_list:
                    item = {}
                    # print(data["com_new_name"])
                    # print(data["com_new_url"])
                ...
                break
    def parse_content(self, response, label):
        item = {}
        label = label

        html = etree.HTML(response)
        # for str in  html.xpath("//h1/text()"):
        #     str.replace("\t","")
